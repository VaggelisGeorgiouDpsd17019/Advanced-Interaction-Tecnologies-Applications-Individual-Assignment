...
Το εξής έγγραφο αποτελεί την τελική αναφορά για το μάθημα «Προηγμένες Τεχνολογίες Αλληλεπίδρασης και Εφαρμογές» από τον φοιτητή Γεωργίου Βαγγέλη με αριθμό μητρώου dpsd17019. Παρακάτω διαφαίνονται τα ερωτήματα και οι απαντήσεις τους που έχουν πραγματοποιηθεί με τη γλώσσα της Processing.

# Summary

Περιεχόμενα αυτού του εγγράφου αποτελούν οι εργασίες που είχαμε κατά τη διάρκεια του εξαμήνου και οι απαντήσεις αυτών, πλην των εργασιών bonus. Εκτός από τις απαντήσεις στα υποερωτήματα κάθε εργασίας, βρίσκονται συμπληρωμένα ορισμένα σχόλια και παρατηρήσεις που σημειώθηκαν κατά τη διάρκεια υλοποίησης των εργασιών. Τα περιεχόμενα του εξής εγγράφου αναγράφονται παρακάτω:

* Παραδοτέο 1: (Έως 4/11/2022)
	* Video Capture
	* Recorded video
	* QR Code
	* QR Code - Camera Read
...
* Παραδοτέο 2: (Έως 16/12/2022)
	* Background Removal
	* Motion Detection
	* Background Substraction - Library use
	* Object Tracking
	
* Παραδοτέο 3: Image Processing Application (Έως 13/1/2023)
...
__Βήμα4:__ Το βήμα αυτό αφορά τον πρώτο τύπο επεξεργασίας της εικόνας, δηλαδή την απλή εμφάνισή της αλλά και την δυνατότητα να κάνει rotate γύρω από τον εαυτό της, μόνο όταν εμφανίζεται το object 0 στον simulator, ή το fiducial 0 στην οθόνη. 
...
__Βήμα5:__ Το βήμα αυτό αφορά την αλλαγή του transparency της εικόνας, μέσω του object/fiducial 1. Για την υλοποίηση αυτού του τρόπου επεξεργασίας χρησιμοποίησα τις εντολές tint() και map().
...
Στις if αυτού του τρόπου επεξεργασίας έβαλα στη θέση του tobj.getAngle() το tobj.getRotationSpeed(). Παρατήρησα ότι στην περίπτωση του tobj.getAngle() η εικόνα αυξομειωνόταν, όχι μόνο όταν έστρεφα το fiducial, αλλά ακόμα και όταν το μετακινούσα. 

Ο κώδικας στην αρχή ήταν ως εξής: 

If (tobj.getAngle( ) > PI/2) { αύξησε την τιμή του offset, αύξησε το ImgWidth και το ImgHeight της εικόνας. Αυτό είχε ως αποτέλεσμα, κάθε φορά που η γωνία του fiducial ήταν μεγαλύτερη των 90 μοιρών, να επηρεάζεται το μέγεθος της εικόνας, ακόμα και αν δεν το έστρεφα εκείνη τη στιγμή. 

Η παραπάνω αστοχία επιλύθηκε όταν αντικατέστησα το tobj.getAngle() με tobj.getRotationSpeed(), του οποίου η χρήση στον κώδικα γίνεται ως εξής: if (tobj.getRotationSpeed() > 0) { αύξησε το μέγεθος της εικόνας} και : if (tobj.getRotationSpeed() < 0) { μείωσε το μέγεθος της εικόνας }

Ο κώδικας για το fiducial 3 φαίνεται στην παρακάτω εικόνα:

![image](https://user-images.githubusercontent.com/115811465/212137910-e1e204ed-ffcd-4503-9c85-249d72016ed4.png)

Αρχικά έδωσα την τιμή 1 στην μεταβλητή offset. Κατόπιν, όρισα στην πρώτη if τα εξής: offset = offset + 5; ώστε να υπάρχει αύξηση της τιμής offset όταν η ταχύτητα περιστροφής του fiducial 3 είναι μεγαλύτερη του 0. Ανάθεσα επίσης τις τιμές “ImgWidth + offset” και “ImgHeight + offset” στο width και στο height της εικόνας αντίστοιχα, ώστε να εμφανίζεται το εφέ του ζουμ.

Όταν η ταχύτητα περιστροφής  του fiducial 3 είναι κάτω από 0 ανάθεσα τις εξής τιμές. ‘’offset – 5” για την offset, ώστε να γίνεται μείωση της τιμής της. Μετά έγραψα τον ακριβώς ίδιο κώδικα για το ImgWidth και ImgHeight με την προηγούμενη if.
...
Η περίπτωση αλλαγής του transparency της εικόνας λειτουργεί αποτελεσματικά όταν εμφανίζω συνδυαστικά τα fiducials/objects 0 & 1.
Παρ' όλα αυτά εμφανίζεται η παρακάτω αστοχία όταν επιλέγω να εμφανίσω ταυτόχρονα τα fiducials/objects 0, 1 & 2 (αλλαγή των r,g,b): 

--> Εάν εμφανίσω κατά σειρά τα fiducials/objects 0, 1 και 2, τότε δεν θα μπορέσω να πετύχω αλλαγή στις τιμές r,g,b με διατηρημένη την αλλαγή του transparency από το fiducial/object 1. Συγκεκριμένα, εμφανίζω δηλαδή το fiducial 2 στην οθόνη, το στρέφω και δεν απεικονίζεται καμία αλλαγή στο χρώμα.

--> Εάν όμως αφαιρέσω το fiducial 1 από τον simulator ή την οθόνη, τότε θα εμφανιστεί αλλαγή στα r,g,b χρώματα με αλλαγμένο το transparency. Εάν μετά επαναφέρω το fiducial 1 στον simulator ή στην οθόνη, τότε μπορώ να αλλάξω ταυτόχρονα και το transparency και τις τιμές r,g,b.

----------------------------------------------------------------------------------------------------------------------------------------------------------------------

__Σε ποια φάση της σχεδίασης και ανάπτυξης του υλικό/λογισμικού της διάδρασης θα διαλέγατε την κανονική κάμερα ή τον προσομοιωτή?__

Ένα προτέρημα του simulator είναι ότι κάνει την διαδικασία πιο γρήγορη, συνεπώς ο designer μπορεί να χρησιμοποιήσει το πρόγραμμα συνδυαστικά με τον tuio simulator για να εξοικονομήσει χρόνο. O simulator περιέχει, όχι μόνο τον απαραίτητο κώδικα, αλλά και εύκολη, εύχρηστη διεπαφή. Περιέχει ένα αχνό κύκλο, ο οποίος αναπαριστά την οθόνη ή το τραπέζι στο οποίο θα τοποθετούνται τα fiducials. Περιέχει ακόμα έτοιμα objects τα οποία αναπαριστούν τα printed fiducials. Έτσι ο designer μπορεί να χρησιμοποιήσει τον simulator κατά τη διάρκεια της πρωτοτυποποίησης για να τεστάρει τον κώδικά του άμεσα και γρήγορα.

Από την άλλη, η χρήση της κάμερας και των τυπωμένων fiducials κάνουν τη διαδικασία πιο διασκεδαστική και μπορεί να χρησιμοποιηθεί σε μετέπειτα στάδια της σχεδίασης, όταν έχει επιβεβαιωθεί ότι ο κώδικας λειτουργεί αποτελεσματικά.
...
Η γλώσσα της Processing αποτελεί μια γλώσσα εισαγωγής στον προγραμματισμό. Διαθέτει τόσο δικές της έτοιμες εντολές, όσο και κλασική γραφή κώδικα, η οποία μπορεί να χρησιμοποιηθεί και από άλλες γλώσσες προγραμματισμού.
