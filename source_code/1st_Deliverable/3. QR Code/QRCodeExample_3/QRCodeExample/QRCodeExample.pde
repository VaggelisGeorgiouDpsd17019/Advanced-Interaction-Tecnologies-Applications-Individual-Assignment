...
PImage img; //prosthiki grammis kwdika gia parousiash eikonas

import processing.video.*;
import qrcodeprocessing.*;

Capture video;                                 // instance of the video capture library
String statusMsg = "";                         // a string to return messages:
...
void setup() {
  ...
  //video.start();
  img = loadImage("QRCODE_DPSD17019_GITHUB.png"); //prosthiki grammis kwdika gia parousiash eikonas
...
void decoderEvent(Decoder decoder) {
  ...
  println(statusMsg); //prosthiki grammis kwdika gia to kwdika tou qr code
  decoder.decodeImage(img); //prosthiki grammis kwdika gia decoding apo to tutorial tou QR CODE - kane sxolio ti grammi kwdika
}
...
void draw() {
  ...
  image(img, 0, 0); //prosthiki grammis kwdika gia parousiash eikonas
  ...
  if (decoder.decoding()) {
   ...
   decoder.decodeImage(img); //prosthiki grammis kwdika gia decoding apo to tutorial tou QR CODE
  }
}

//prosthiki neou kwdika gia anoigma link 
//source: https://discourse.processing.org/t/open-a-link-on-a-browser-with-keypressed/6740/2
...
void keyReleased() {
  ...
  case 'f':    // f runs a test on a file
    PImage preservedFrame = loadImage("QRCODE_DPSD17019_GITHUB.png"); //evala edw tin eikona me to diko mou QR Code
    // Decode file
